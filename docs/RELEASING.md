...
1. **Changelog and version**
   ...
   - Record `npm --cache /private/tmp/npm-cache-codex pack --dry-run ./lib` and `npm --cache /private/tmp/npm-cache-codex pack --dry-run ./regions` output when generated region data changes. Latest region-data validation for the 23-country optional package: optional `@react-svg-worldmap/regions@0.1.0` dry-run is 2.8 MB packed / 10.2 MB unpacked with 7 files after generated SVG paths are rounded to hundredth-pixel precision and the MIT license is included. Latest gradual-disclosure validation for the core package: reduced country tier is 1,574,896 bytes source with `4000` quantization and 91.0% source reduction; detailed country tier is 2,430,645 bytes source with `10000` quantization and 86.1% source reduction; `react-svg-worldmap@2.0.2` dry-run is 1.2 MB packed / 10.4 MB unpacked with 6 files. Latest zoom-control and rendering-layer validation keeps `react-svg-worldmap@2.0.2` at 1.2 MB packed / 10.4 MB unpacked with 6 files after using bottom-right plus/minus controls, explicit SVG layer order, removing visible reset and below-map region list output, removing the `showRegionList` API, and keeping clicked-point double-click zoom. Parse-guard tests validate the reduced tier is parsed on initial imports while the detailed tier remains unparsed until the `2x` loading path, and region provider tests validate optional region loading is deferred until `4x`. Package contents remain README, package metadata, built JS, and type outputs with no hosted tile, Google Maps, raster basemap, or other runtime map-service dependency added.
