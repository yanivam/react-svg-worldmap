...
1. **Changelog and version**
   ...
   - Record `npm --cache /private/tmp/npm-cache-codex pack --dry-run ./lib` and `npm --cache /private/tmp/npm-cache-codex pack --dry-run ./regions` output when generated region data changes. Latest region-data validation for the 23-country optional package: optional `@react-svg-worldmap/regions@2.1.0` includes README, CHANGELOG, license, contributing guidance, code of conduct, built JS, and type outputs. Latest gradual-disclosure validation for the core package: reduced country tier is 1,574,896 bytes source with `4000` quantization and 91.0% source reduction; detailed country tier is 2,430,645 bytes source with `10000` quantization and 86.1% source reduction. Latest shared-asset validation for `react-svg-worldmap@2.1.0`: country topology still ships inside the core package, but the public ESM and CommonJS entry files now reference package-local `map-assets/*.cjs` files instead of duplicating the same large topology payload in both entry bundles. The latest core dry-run is 588.4 kB packed, 4.2 MB unpacked, and 8 files. Published JavaScript intentionally has no `sourceMappingURL` comments because source maps are not included in the npm package. Latest zoom-control and rendering-layer validation keeps staged zoom rendering, explicit SVG layer order, bottom-right plus/minus controls, country-border strengthening for visible regions, and concise `Region, Country` region hover labels. Parse-guard tests validate the reduced tier is parsed on initial imports while the detailed tier remains unparsed until the `2x` loading path, and region provider tests validate optional region loading is deferred until `4x`. Package contents remain README, package metadata, built JS, types, and package-local country map assets with no hosted tile, Google Maps, raster basemap, or other runtime map-service dependency added.
   - For package artifact changes, run `yarn inspect:package` after `yarn build:package`. The inspection must pass the core size budgets, confirm `map-assets/countries-reduced.topo.cjs` and `map-assets/countries-detailed.topo.cjs` are present, and confirm published JavaScript has no missing source-map references.
