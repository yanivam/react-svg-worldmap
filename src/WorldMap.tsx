...
import React, {
  MouseEvent, FC, createRef, useState, useLayoutEffect,
} from 'react'
...
interface IProps {
  ...
  onClickFunction?:
  // eslint-disable-next-line no-unused-vars
  (event: MouseEvent<SVGElement, Event>, countryName: string,
    // eslint-disable-next-line no-unused-vars
...
export const WorldMap: FC<IProps> = (props) => {
  ...
  const defaultCountryStyle = (context : ICountryContext) => {
    ...
    + 0.6 * ((contextCountryValue - context.minValue) / (context.maxValue - context.minValue))
    const style = {
      fill: context.color, fillOpacity: contextCountryValue === 0 ? contextCountryValue : opacityLevel, stroke: borderColor, strokeWidth: 1, strokeOpacity, cursor: 'pointer',
    }
    return style
  }
  ...
  const pathsAndToolstips = geoData.features.map((feature, idx) => {
    ...
    const style = props.styleFunction && isHighlight ? props.styleFunction({
      country: isoCode,
    ...
    if (props.tooltipTextFunction && countryValueMap[isoCode]) {
      tip = props.tooltipTextFunction(countryName, isoCode, countryValueMap[isoCode].toString(), valuePrefix || '', valueSuffix || '')
  ...
  // build tooltips
  const highlightedTooltips = pathsAndToolstips.map((entry) => entry.highlightedTooltip)

  // Render the SVG
  return (
    <div style={{
      backgroundColor, height: 'auto', width: 'auto', padding: '0px', margin: '0px',
    }}>
      {title}
